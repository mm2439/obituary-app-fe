...
import ObituaryPhotoUpload from "@/app/components/upload/ObituaryPhotoUpload";
import FileUpload from "@/app/components/upload/FileUpload";

const AddObituary = ({ set_Id, setModal }) => {
  const router = useRouter();
  const { user, isAuthenticated } = useAuth();
  const { logObituaryCreate } = useActivity();
  ...
  const handleSubmit = async () => {
    ...
    if (!isAuthenticated() || !user) {
      toast.error("Please log in to create obituaries.");
      return;
    }
    ...
    try {
      ...
      // Try using the new Supabase service first
      try {
        const obituaryData = {
          name: firstName,
          sirName: lastName,
          ...
          funeralDate: funeralDate?.toISOString(),
          ...
          deathReport: uploadedDeathReport,
          deathReportExists: isDeathReportConfirmed,
          events: events
        };

        if (dataExists) {
          // Update existing obituary
          const supabaseResult = await ObituaryService.updateObituary(user.id, obituaryData, user.id);
          if (supabaseResult.success) {
            response = supabaseResult.data;
            toast.success(supabaseResult.message);
          } else {
            throw new Error(supabaseResult.error);
          }
        } else {
          // Create new obituary
          const supabaseResult = await ObituaryService.createObituary(obituaryData, user.id);
          if (supabaseResult.success) {
            response = supabaseResult.data;
            toast.success(supabaseResult.message);
          } else {
        ...
        console.warn('Supabase service failed, falling back to legacy service:', supabaseError);
