...
const AddObituary = ({ set_Id, setModal }) => {
  ...
  useEffect(() => {
    ...
    if (!storedUser) {
      ...
      setUser(parsedUser);
       if (parsedUser?.id) {
      console.log("fetchUserWorks..")
      fetchUserObituary(parsedUser.id);
    }
    }
  }, [router]);
...
const fetchUserObituary = async (userId) => {
  try {
    ...
    if (response) { // If user has an existing obituary
      // Basic information
      setDataExists(true);
                    ...
                    <div className="flex flex-row mobile:gap-x-[11px] gap-x-[32px] gap-y-[8px] flex-wrap">
                      ...
                      <div className="flex relative">
                        <ModalDropBox
                          ...
                          isSelectText={
                            event.eventDate ? getMonth(event.eventDate) + 1 : ""
                          }
                        />
                      ...
                      {openPicker === `eventYear_${index}` && (
                        <div className="absolute mt-12 left-[56%] bg-white border rounded shadow-lg z-10">
                          <DatePicker
                            selected={event.eventDate}
                            onChange={(date) => {
                              const currentDate = event.eventDate || new Date();
                              const updatedDate = new Date(
                                date.getFullYear(),
                                currentDate.getMonth(),
                                currentDate.getDate()
