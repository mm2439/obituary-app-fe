...
import { isAuthenticatedSync, getUserSync } from "@/utils/authUtils";
...
const Registration = () => {
  ...
  // Use the new auth system with backward compatibility
  ...
  useEffect(() => {
    // Use synchronous check for immediate redirect
    ...
    // Also check with the new auth system
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // Use the new Supabase auth system
      const result = await auth.register({
        email: inputValueEmail,
        password: inputValueGeslo,
        role: "User",
        name: inputValueChooseUsername || ""
      });
      ...
      if (result.success) {
        ...
        setInputValueEmail("");
        setInputValueGeslo("");
        setInputValueConfirmGeslo("");
        setInputValueChooseUsername("");
      } else {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Use the new Supabase auth system with backward compatibility
      ...
      if (result.success && result.user) {
        ...
        // Navigate using the existing navigation logic
        const role = result.user.role;
        const slugKey = result.user.slugKey;
          ...
          {activeDiv === "register" && (
            <div className=" w-[429px] mobile:w-[297px] h-[48px] mt-[20px] mx-auto shadow-custom-light-dark-with-white">
              <button
                ...
                disabled={isLoading}
                className="flex w-full h-full items-center justify-center text-[16px] leading-[24px] font-variation-customOpt16 text-[#FFFFFF] bg-gradient-to-b from-[#0D94E8] to-[#1860A3] rounded-lg disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          {activeDiv === "login" && (
            <div className="w-[429px] mobile:w-[297px] h-[48px] mt-[20px] mx-auto shadow-custom-light-dark-with-white">
              <button
                ...
                onClick={handleLogin}
                disabled={isLoading}
                className="flex w-full h-full bg-white items-center justify-center text-[16px] leading-[24px] font-variation-customOpt16 text-[#FFFFFF] bg-gradient-to-b from-[#0D94E8] to-[#1860A3] rounded-lg disabled:opacity-50 disabled:cursor-not-allowed"
              >
